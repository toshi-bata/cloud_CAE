...
static char _pcCEWModelDir[_MAX_PATH] = { '\0' };
	...
	size_t *upload_data_size, void **con_cls)
{
	...
	if (strcmp(method, "POST") == 0)
	{
		...
		else if (NULL != con_info->sessionId)
		{
			...
			else if (0 == strcmp(url, "/DownloadCAE"))
			{
				std::string fileName;
				if (!paramStrToStr("currentModel", fileName)) return MHD_NO;

				int iRet = _pPsProcess->SaveCAE(fileName);
			...
			else if (0 == strcmp(url, "/VolumeMesh"))
			{
				PK_BODY_t body;
				double edgeLen;
				if (!paramStrToInt("body", body)) return MHD_NO;
				if (!paramStrToDbl("meshSize", edgeLen)) return MHD_NO;

				int iRet = -1;
#ifndef _DEBUG
				iRet = _pPsProcess->VolumeMesh(body, edgeLen, ACTIVE_SESSION.c_str());
#endif
				return sendResponseSuccess(connection);
			}
			else if (0 == strcmp(url, "/Solve"))
			{
				double loadN, density, young, poisson;
				double *loadVec = NULL;
				int constCnt, loadCnt;
				PK_ENTITY_t *constFaces = NULL, *loadFaces = NULL;

				if (!paramStrToDbl("loadN", loadN)) return MHD_NO;
				if (!paramStrToXYZ("loadVec", loadVec)) return MHD_NO;
				if (!paramStrToDbl("density", density)) return MHD_NO;
				if (!paramStrToDbl("young", young)) return MHD_NO;
				if (!paramStrToDbl("poisson", poisson)) return MHD_NO;
				if (!paramStrToPkEntities("constFaces", constCnt, constFaces)) return MHD_NO;
				if (!paramStrToPkEntities("loadFaces", loadCnt, loadFaces)) loadCnt = 0;

				int iRet = -1;
#ifndef _DEBUG
				iRet = _pPsProcess->Solve(loadN, loadVec, density, young, poisson, constCnt, constFaces, loadCnt, loadFaces, ACTIVE_SESSION.c_str());
#endif
				return sendResponseSuccess(connection);
			}
		}
...
int main(int argc, char ** argv)
{
	...
	// Get CEW UG model dir
	GetEnvironmentVariablePath("CEW_UG_MODEL_PATH", _pcCEWModelDir, true);
	if (0 == strlen(_pcCEWModelDir))
		return false;

	_pPsProcess = new CPsProcess(_pcWorkingDir, _pcCEWModelDir);
