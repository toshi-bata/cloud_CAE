class Main {
    ...
    start (port, viewerMode, modelName, reverseProxy, startCAE) {
        this._CAE = startCAE;
        this._requestServerProcess(port, reverseProxy).then(() => {
            ...
            if (this._CAE) {
                // Start CEETRON UG Viewer
                this._ceeViewer = new CEWUgViewer();
                this._ceeViewer.startApp(reverseProxy);

                // Read material properties
                const fileName = "json/materials.json";
                $.get(fileName, () => {
                    $.getJSON(fileName, (data) => {
                        if (data.materials) {
                            ...
                            for (let i = 0; i < this._materials.length; i++) {
                                ...
                                $("#materialSelect").append($("<option>").val(i).text(material.name));
                                if (1 == i) {
        ...
        createViewer(viewerMode, modelName, "container", reverseProxy).then((hwv) => {
            ...
            this._viewer.setCallbacks({
                sceneReady: () => {
                    ...
                    if (this._CAE) {
                        this._viewer.view.setBackgroundColor(new Communicator.Color(176, 196, 222), new Communicator.Color(176, 196, 222));
    ...
    _initEvents () {
        ...
        $(".simpleCmd").on("click", (e) => {
            ...
            switch(command) {
                ...
                case "DownloadCAE": {
                    const currentModel = this._ceeViewer.getCurrentModel();
                    if (null != currentModel) {
                        const params = { currentModel: currentModel };
                        this._psServerCaller.CallServerPost(command, params).then(() => {
                            const fileExt = currentModel.split('.').pop();
                            this._fileDownload(currentModel, "result_model." + fileExt);
                        });
                    }
                } break;
                case "MoveToCAE":           this._moveToCAE();                      break;
                ...
                case "SectionCAE":          this.showCuttingPlaneDialog();          break;
                case "VectorResult":        this.showVectorResultDialog();          break;
                case "ShowNodeAveraged":    this._ceeViewer.toggleNodeAveraged();   break;
                case "ShowMaxValue":        this._ceeViewer.toggleMaxValue();       break;
                case "ReloadCAE": {
                    this._ceeViewer.Reload().then(() => {
                        const scalerResults = this._ceeViewer.getScalerResults();
                        if (null != scalerResults) {
                            let vonMisesId = 0;
                            for (let result of scalerResults) {
                                const id = result.id;
                                const name = result.name;
                                $("#scalerResults").append($("<option>").val(id).text(name));
                                if (0 == vonMisesId) {
                                    if (0 == name.indexOf("vonMises")) {
                                        vonMisesId = id;
                                    }
                                }
                            }
                            this._ceeViewer.showScalarResult(vonMisesId);
                            $('#scalerResults').val(vonMisesId);
                        }
                        else {
                            $("select#scalerResults option").remove();
                        }
                    });
                } break;
        ...
        $(".dlgOkCmd").on("click", (e) => {
            ...
            if(!isOn) {
                ...
                switch (command) {
                    ...
                    case "Solve": {                        
                        $("#info1").show().html("Solve: Stress analysis");
                        $("#constFace").focus();
                        $("#OkBtn").val("Solve");

                        this._nodeSelectOp.setSelectId("constFace", "loadFace");
                        this._nodeSelectOp.setPickConfig(new Communicator.PickConfig(Communicator.SelectionMask.Face));
                        this._nodeSelectOp.setShowNormalArrow([false, true]);
                        const id = this._viewer.operatorManager.indexOf(Communicator.OperatorId.Select);
                        this._viewer.operatorManager.set(this._nodeSelectOpHandle, id);

                        let nodes1 = new Array(0);
                        let nodes2 = new Array(0);
                        let faces1 = new Array(0);
                        let faces2 = new Array(0);
                        let psEntities1 = new Array(0);
                        let psEntities2 = new Array(0);
                        for (let psFaceId of this._constFaceArr) {
                            const ids = this.getFaceNodeId(psFaceId);
                            if (undefined != ids.nodeId) {
                                nodes1.push(ids.nodeId);
                                faces1.push(ids.faceId)
                                psEntities1.push(psFaceId);
                            }
                        }
                        for (let psFaceId of this._loadFaceArr) {
                            const ids = this.getFaceNodeId(psFaceId);
                            if (undefined != ids.nodeId) {
                                nodes2.push(ids.nodeId);
                                faces2.push(ids.faceId)
                                psEntities2.push(psFaceId);
                            }
                        }
                        this._nodeSelectOp.setPreSelection(nodes1, nodes2, faces1, faces2, psEntities1, psEntities2);        
                    } break;
                    case "SetupAnimation": {
                        $("#info1").show().html("Setup Mode Shape Animation");
        ...
        $("#OkBtn").on("click", (e) => {
            ...
            switch (command) {
                ...
                case "Solve": {
                    this._constFaceArr = this._nodeSelectOp.getPsEntities(0);
                    this._loadFaceArr = this._nodeSelectOp.getPsEntities(1);

                    if (this._constFaceArr.length){
                        $("#loadingImage").show();

                        const constNodes = this._nodeSelectOp.getNodes(0);
                        const constFaceIds = this._nodeSelectOp.getEntities(0);
                        for (let faceId of constFaceIds) {
                            this._viewer.model.setNodeFaceColor(constNodes[0], faceId, new Communicator.Color(0, 128 ,128));
                        }
    
                        const loadNodes = this._nodeSelectOp.getNodes(1);
                        const loadFaceIds = this._nodeSelectOp.getEntities(1);
                        for (let faceId of loadFaceIds) {
                            this._viewer.model.setNodeFaceColor(loadNodes[0], faceId, new Communicator.Color(128, 0 ,128));
                        }

                        const loadVal = Number($("#loadVal").val());
                        const vecX = Number($("#loadVecX").val()); 
                        const vecY = Number($("#loadVecY").val()); 
                        const vecZ = Number($("#loadVecZ").val()); 

                        // Material properties
                        const density = $("#density").val(); 
                        const young = $("#young").val(); 
                        const poisson = $("#poisson").val(); 

                        params = {
                            loadN: loadVal, loadVec: [vecX, vecY, vecZ],
                            density: density, young: young, poisson: poisson,
                            constFaces: this._constFaceArr, loadFaces: this._loadFaceArr
                        }
                        this.invokeCAE(command, params);
                    }
                } break;
                case "SetupAnimation": {
        ...
        $(".entityOneClickCmd").on("click", (e) => {
            ...
            if(!isOn) {
                ...
                switch (command) {
                    case "VolumeMesh": {
                        $("#info1").show().html("Volume Mesh: Select a body to volume.");
                        this._entityOneClickOp.setCommand(command, new Communicator.PickConfig(Communicator.SelectionMask.Face));
                    } break;
                    case "DeleteBody": {
        ...
        this._psServerCaller.CallServerPostFile(f).then((res) => {
            if (Object.keys(res).length) {
                if ("CAE" == res.type) {
                    ...
                    this._moveToCAE(this._sessionId + "." + fileType);
                    $("#loadingImage").hide();
                }
                else {
                    // CAD model
    ...
    invokeCAE (command, params) {
        $("#loadingImage").show();

        this._psServerCaller.CallServerPost(command, params).then(() => {
            let modelName;
            if (0 == command.indexOf("VolumeMesh")) {
                modelName = this._sessionId + ".bdf";
            }
            else if (0 == command.indexOf("Solve")) {
                modelName = this._sessionId + ".op2";
            }
            this._moveToCAE(modelName, this._viewer.view.getCamera());
            
            $("#loadingImage").hide();
        }).catch((error) => {
            $("#loadingImage").hide();
            alert("CAE command failed");
        });
    }
    ...
    showVectorResultDialog() {
        let theDialog = $("#myModalDialog");
        let html = "";
        html += '<ol id="selectable">';
        html += '<li class="ui-widget-content">None</li>';
        // let model = this.m_model;
        let model = this._ceeViewer.getModel();
        let theCaseDirectory = model.modelDirectory;
        theCaseDirectory.vectorResultArray.forEach(vecInfo => {
            html += '<li class="ui-widget-content">' + vecInfo.name + '</li>';
        });
        theDialog.html(html);
        let sel = $("#selectable");
        sel.selectable({
            selected: function () {
                $(".ui-selected", this).each(function () {
                    ...
                    let resultId = -1;
                    if (index > 0) {
                        resultId = theCaseDirectory.vectorResultArray[index - 1].id;
                    }
                    let modelSpec = model.modelSpec;
                    modelSpec.vectorResultIdArray = [resultId];
                });
            }
        });
        theDialog.dialog({
            title: "Select vector result",
            modal: false,
            width: 350,
            height: 500,
            position: { my: "right top", at: "left top", of: "#myToolBox" },
            buttons: {}
        });
    }


    _moveToCAE(modelName, hcCamera) {
        if (modelName) {
            this._ceeViewer.openModel(modelName, hcCamera).then(() => {
                $("select#scalerResults option").remove();
                $("#scalerResults").append($("<option>").val(-1).text("None"));
                ...
                $("#ModelMesh").data("on", true).css("background-color", "khaki");
                $(".CAEdefaultOff").data("on", false).css("background-color", "gainsboro");
                $(".CAD_contents").hide();
                $(".CAE_contents").show();
                $("#dataInfo").html("");
                this._viewer.model.resetModelHighlight();
            ...
            this._ceeViewer.setCamera(this._viewer.view.getCamera());
