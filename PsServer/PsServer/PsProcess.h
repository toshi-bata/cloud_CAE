...
{
public:
	CPsProcess(const char *workingDir, const char *CEWModelDir);
	~CPsProcess();
...
public:
	...
	int SaveCAE(std::string fileName);
	void Downloaded();
	...
	int VolumeMesh(const PK_BODY_t body, const double edgelen, const char *modelName);
	int Solve(const double loadN, const double* loadVec, const double density, const double young, const double poisson, 
		const int constCnt, const PK_FACE_t* constFaces, const int loadCnt, const PK_FACE_t* loadFaces, const char *modelName);
