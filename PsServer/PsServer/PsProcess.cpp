...
bool isCeeFile(const char *lowExt)
{
	const char *ceeExts[] = {
		"op2", "bin", "fil", "cgns", "rst", "rth", "rfl", "neu", "unv", "univ", "dat", "bdf", "cas",
		"dat", "inp", "cdb", "stl", "sla", "cgeo", "k", "key d3plot", "d3plot", "ptf", "foam", "vtu",
		"pvd", "pvtu", "vtm", "pvd", "t16", "t19", "neu", "case", "pit", "fg3", "fg2", "fr3", "may",
		"in3", "th3", "don" };

	for (const char* ext : ceeExts)
	{
		if (0 == strcmp(lowExt, ext))
			return true;
	}
...
CPsProcess::CPsProcess(const char *workingDir, const char *CEWModelDir)
...
int CPsProcess::SaveCAE(std::string fileName)
{
	char filePathFrom[_MAX_PATH];
	sprintf(filePathFrom, "%s%s", m_pcWorkingDir, fileName.c_str());


	sprintf(m_pcFilePathForDL, "..\\%s", fileName.c_str());

	copy_file(filePathFrom, m_pcFilePathForDL);
...
int CPsProcess::VolumeMesh(const PK_BODY_t body, const double edgeLen, const char *activeSession)
{
	int iRet = -1;

#ifndef _DEBUG
	CSAMProcess samProcess = CSAMProcess(m_pcWorkingDir, m_pcCEWModelDir);

	iRet = samProcess.PsSolidToTetMesh(1, &body, edgeLen / m_dScale, activeSession);
#endif
	return iRet;
}

int CPsProcess::Solve(const double loadN, const double* loadVec, const double density, const double young, const double poisson,
	const int constCnt, const PK_FACE_t* constFaces, const int loadCnt, const PK_FACE_t* loadFaces, const char *activeSession)
{
	...
	iRet = samProcess.Solve(loadPa, loadVec, density, young, poisson, constCnt, constFaces, loadCnt, loadFaces, activeSession);
#endif
	return iRet;
}
